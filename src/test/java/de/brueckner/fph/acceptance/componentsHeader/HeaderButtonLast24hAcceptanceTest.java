...
import static de.brueckner.fph.managers.ContentManager.LAST_24H_HEADER_BUTTON;
import static de.brueckner.fph.util.DateUtils.DEFAULT_SHORT_DATE_FORMAT;

public class HeaderButtonLast24hAcceptanceTest extends AbstractApplicationSettingsTest {
    private static final Logger logger = LoggerFactory.getLogger(HeaderButtonLast24hAcceptanceTest.class);

    @Test
    public void headerButtonLast24hAcceptance() {
        logger.info("headerButtonLast24hAcceptance");
        applicationManager.loginPage(LoginCredentials.USER_DEV.getUsername(), LoginCredentials.USER_DEV.getPassword());

        WebElement last24H = getWebDriver().findElement(By.cssSelector(LAST_24H_HEADER_BUTTON));
        last24H.click();
        contentManager.waitContentSpinner();

        //check if the selection is ok in breadcrum
        breadcrumbManager.checkBreadcrumb(Arrays.asList("Frd", "Last 24h"));
